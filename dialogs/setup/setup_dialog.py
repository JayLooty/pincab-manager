...
class SetupDialog:
    ...
    def __validate(self):
        ...
        for media in Media:
            ...
            match media:
                case Media.TOPPER:
                    if self.media_screen_number_topper_screen_combo.current() > 0:
                        screen_number = \
                            self.media_screen_number_topper_screen_combo.current() - 1
                case Media.BACKGLASS:
                    if self.media_screen_number_backglass_screen_combo.current() > 0:
                        screen_number = \
                            self.media_screen_number_backglass_screen_combo.current() - 1
                case Media.FULL_DMD:
                    if self.media_screen_number_full_dmd_screen_combo.current() > 0:
                        screen_number = \
                            self.media_screen_number_full_dmd_screen_combo.current() - 1
                case Media.PLAYFIELD:
                    if self.media_screen_number_playfield_screen_combo.current() > 0:
                        screen_number = \
                            self.media_screen_number_playfield_screen_combo.current() - 1
    ...
    def __create_general_components(self):
        ...
        self.combo_lang.bind("<<ComboboxSelected>>", self.__on_entry_changed)

        # Create folder vpinball selection
        vpinball_path_frame = tk.Frame(general_frame)
        vpinball_path_frame.pack(
            side=tk.TOP,
            fill=tk.X,
            pady=Constants.UI_PAD_SMALL
        )
        label_vpinball_path = tk.Label(
            vpinball_path_frame,
            text=Context.get_text('vpinball_path')
        )
        label_vpinball_path.pack(
            side=tk.LEFT,
            padx=Constants.UI_PAD_SMALL
        )
        self.entry_vpinball_path_var = tk.StringVar()
        self.entry_vpinball_path_var.trace_add(
            "write",
            self.__on_entry_changed
        )
        self.entry_vpinball_path = tk.Entry(
            vpinball_path_frame,
            textvariable=self.entry_vpinball_path_var,
            width=40
        )
        self.entry_vpinball_path.insert(
            0,
            Context.get_vpinball_path()
        )
        self.entry_vpinball_path.pack(
            side=tk.LEFT,
            padx=Constants.UI_PAD_SMALL
        )
    ...
    def __create_media_components(self):
        ...
        self.media_screen_number_topper_screen_combo = ttk.Combobox(
            media_topper_frame,
        ...
        if current_media.value in Context.get_screen_number_by_media() \
            ...
            self.media_screen_number_topper_screen_combo.set(
        ...
        self.media_screen_number_topper_screen_combo.config(state="readonly")
        self.media_screen_number_topper_screen_combo.bind(
            "<<ComboboxSelected>>", self.__on_entry_changed
        )
        self.media_screen_number_topper_screen_combo.pack(
            side=tk.RIGHT,
            padx=Constants.UI_PAD_SMALL
        )
        media_screen_number_topper_screen_label = tk.Label(
            media_topper_frame,
            text=Context.get_text('setup_screen_number')
        )
        media_screen_number_topper_screen_label.pack(
            side=tk.RIGHT
        )
        ...
        self.media_screen_number_backglass_screen_combo = ttk.Combobox(
            media_backglass_frame,
            values=screen_numbers_values
        )
        if current_media.value in Context.get_screen_number_by_media() \
                and Context.get_screen_number_by_media()[current_media.value] >= 0:
            self.media_screen_number_backglass_screen_combo.set(
                value=screen_numbers_values[Context.get_screen_number_by_media()[
                    current_media.value] + 1]
            )
        else:
        ...
        self.media_screen_number_backglass_screen_combo.config(
            state="readonly")
        self.media_screen_number_backglass_screen_combo.bind(
            "<<ComboboxSelected>>", self.__on_entry_changed
        )
        self.media_screen_number_backglass_screen_combo.pack(
            side=tk.RIGHT,
            padx=Constants.UI_PAD_SMALL
        )
        media_screen_number_backglass_screen_label = tk.Label(
            media_backglass_frame,
            text=Context.get_text('setup_screen_number')
        )
        media_screen_number_backglass_screen_label.pack(
            side=tk.RIGHT
        )
        ...
        self.media_screen_number_full_dmd_screen_combo = ttk.Combobox(
            media_full_dmd_frame,
            values=screen_numbers_values
        )
        if current_media.value in Context.get_screen_number_by_media() \
                and Context.get_screen_number_by_media()[current_media.value] >= 0:
            self.media_screen_number_full_dmd_screen_combo.set(
                value=screen_numbers_values[Context.get_screen_number_by_media()[
                    current_media.value] + 1]
            )
        else:
        ...
        self.media_screen_number_full_dmd_screen_combo.config(state="readonly")
        self.media_screen_number_full_dmd_screen_combo.bind(
            "<<ComboboxSelected>>", self.__on_entry_changed
        )
        self.media_screen_number_full_dmd_screen_combo.pack(
            side=tk.RIGHT,
            padx=Constants.UI_PAD_SMALL
        )
        media_screen_number_full_dmd_screen_label = tk.Label(
            media_full_dmd_frame,
            text=Context.get_text('setup_screen_number')
        )
        media_screen_number_full_dmd_screen_label.pack(
            side=tk.RIGHT
        )
        ...
        self.media_screen_number_playfield_screen_combo = ttk.Combobox(
            media_playfield_frame,
            values=screen_numbers_values
        )
        if current_media.value in Context.get_screen_number_by_media() \
                and Context.get_screen_number_by_media()[current_media.value] >= 0:
            self.media_screen_number_playfield_screen_combo.set(
                value=screen_numbers_values[Context.get_screen_number_by_media()[
                    current_media.value] + 1]
            )
        else:
        ...
        self.media_screen_number_playfield_screen_combo.config(
            state="readonly")
        self.media_screen_number_playfield_screen_combo.bind(
            "<<ComboboxSelected>>", self.__on_entry_changed
        )
        self.media_screen_number_playfield_screen_combo.pack(
            side=tk.RIGHT,
            padx=Constants.UI_PAD_SMALL
        )
        media_screen_number_playfield_screen_label = tk.Label(
            media_playfield_frame,
            text=Context.get_text('setup_screen_number')
        )
        media_screen_number_playfield_screen_label.pack(
            side=tk.RIGHT
        )
